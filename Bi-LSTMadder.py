...
def generate_binary_string(max_len):
  bits=[]
...
def random_sum_pairs(n_examples, n_numbers, largest):
	...
	for i in range(n_examples):
		in_pattern = [generate_binary_string(ceil(log2(largest+1))) for _ in range(n_numbers)]
...
print(n_chars)
model = Sequential()
model.add(Bidirectional(LSTM(100, return_sequences=False), input_shape=(n_in_seq_length, n_chars), merge_mode='concat'))
model.add(RepeatVector(n_out_seq_length))
model.add(Bidirectional(LSTM(50, return_sequences=True), input_shape=(n_in_seq_length, n_chars), merge_mode='concat'))
model.add(TimeDistributed(Dense(n_chars, activation='softmax')))
model.compile(loss='categorical_crossentropy', optimizer='adam',metrics=['accuracy'])
print(model.summary())
